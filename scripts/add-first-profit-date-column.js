...
async function addFirstProfitDateColumn() {
  try {
    ...
    const sqlFilePath = path.join(
      __dirname,
      "add-first-profit-date-column.sql"
    );
    const sqlContent = fs.readFileSync(sqlFilePath, "utf8");

    console.log(
      "🚀 Setting up complete investments tables and 24-hour profit system..."
    );
    ...
    console.log(
      "✅ Add the first_profit_date column for 24-hour profit scheduling"
    );
    ...
    console.log(
      "The 24-hour profit system is ready to use after running the SQL!"
    );
  } catch (error) {
