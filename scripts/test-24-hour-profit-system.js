...
async function test24HourProfitSystem() {
  ...
  try {
    ...
    console.log(`✅ Admin approved deposit and created investment: ${investment.id}`);
    ...
    if (verifyInvestment.first_profit_date) {
      console.log(`✅ First profit date set: ${verifyInvestment.first_profit_date}`);
    } else {
    ...
    // Apply the return
    const { error: returnError } = await supabase.rpc('increment_user_balance', {
      user_id: testUser.user.id,
    ...
    console.log(`✅ Applied first profit of $${expectedProfit} to user balance`);
    ...
    // Delete test data
    await supabase.from('investment_returns').delete().eq('investment_id', investment.id);
