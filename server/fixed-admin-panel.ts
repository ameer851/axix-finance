...
function basicAuthCheck(req: AdminRequest, res: Response, next: Function) {
...
function basicAdminCheck(req: AdminRequest, res: Response, next: Function) {
...
// Extending the Request type with our custom properties
interface AdminRequest extends Omit<Request, 'user'> {
...
export function createAdminRouter(): Router {
  ...
  router.get('/stats/dashboard', async (req: AdminRequest, res: Response) => {
    try {
      ...
      res.status(500).json({ success: false, message: 'Failed to get dashboard stats', error: isError(error) ? error.message : 'Unknown error' });
    }
  });
  
  // Get recent activity for the admin dashboard
  router.get('/stats/recent-activity', async (req: AdminRequest, res: Response) => {
    try {
  ...
  router.get('/transactions', async (req: AdminRequest, res: Response) => {
    try {
      ...
      const transactionsPromise = storage.getTransactions({ 
        limit, 
        offset, 
        search,
        status,
      ...
      res.status(500).json({ success: false, message: 'Failed to get transactions', error: isError(error) ? error.message : 'Unknown error' });
    }
  });
  ...
  router.get('/deposits/pending', async (req: AdminRequest, res: Response) => {
    try {
      ...
      const pendingDepositsPromise = storage.getPendingDeposits({ limit, offset });
      
      // Get total count
      const countPromise = storage.getPendingDepositCount();
      
      // Wait for both promises
      const [pendingDeposits, count] = await Promise.all([pendingDepositsPromise, countPromise]);
      ...
      res.status(500).json({ success: false, message: 'Failed to get pending deposits', error: isError(error) ? error.message : 'Unknown error' });
    }
  });

  // Ensure proper error handling and response structure
  router.post('/deposits/:id/approve', async (req: AdminRequest, res: Response) => {
    try {
      ...
      try {
        ...
        console.error(`Database error getting transaction: ${isError(dbError) ? dbError.message : 'Unknown error'}`);
      ...
      let updated;
      try {
        updated = await storage.updateTransactionStatus(depositId, 'completed');
      } catch (updateError) {
        console.error(`Error updating transaction status: ${isError(updateError) ? updateError.message : 'Unknown error'}`);
        return res.status(500).json({ success: false, message: 'Error updating deposit status' });
      }
  ...
  router.get('/audit-logs', async (req: AdminRequest, res: Response) => {
    try {
      ...
      try {
        ...
        console.warn('Failed to get audit logs from database, using demo data:', dbError);
  ...
  router.get('/stats/active-visitors', async (req: AdminRequest, res: Response) => {
    try {
      ...
      try {
        ...
        console.warn('Failed to get active visitors from database, using demo data:', dbError);
        
        // Generate demo visitors data if the database query failed
        visitors = [
          { id: 1, username: 'user1', lastActive: new Date(), currentPath: '/dashboard' },
          { id: 2, username: 'user2', lastActive: new Date(), currentPath: '/portfolio' },
          { id: 3, username: 'user3', lastActive: new Date(), currentPath: '/deposit' },
          { id: 4, username: 'guest_user', lastActive: new Date(), currentPath: '/' },
        ];
      }
      ...
      res.status(500).json({ success: false, message: 'Failed to get active visitors', error: isError(error) ? error.message : 'Unknown error' });
    }
  });
