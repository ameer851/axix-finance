...
pool.on("error", (err: PostgresError) => {
  ...
  if (err.code === "ETIMEDOUT" || err.code === "ECONNREFUSED") {
    console.log(
      "Database connection timed out or was refused. Will retry automatically."
    );
  }
});
  ...
  try {
    ...
    if (pgError.code) {
      ...
      if (pgError.code === "ETIMEDOUT") {
        console.error(
          "Database connection timed out. Please check if the database server is running and accessible."
        );
      } else if (pgError.code === "ECONNREFUSED") {
        console.error(
          "Database connection refused. Please check if the database server is running on the specified host and port."
        );
    ...
    // Set global flag but don't stop the application
    global.dbConnectionIssues = true;
    console.log(
      "🔄 Application will continue with limited functionality using Supabase client directly"
    );
    return false;
  }
}
...
try {
  pool.query("SELECT NOW()", (err, res) => {
    if (err) {
      ...
      console.error(
        "This might affect authentication and other database-dependent features."
      );
      if (err.message.includes("connect ECONNREFUSED")) {
        console.error(
          "Make sure your PostgreSQL database is running on the correct port (currently set to 5000)."
        );
      }
    } else {
