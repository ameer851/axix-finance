...
const supabaseUrl = process.env.SUPABASE_URL || 'https://wvnyiinrmfysabsfztii.supabase.co';
const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Ind2bnlpaW5ybWZ5c2Fic2Z6dGlpIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MzA5NDM2NywiZXhwIjoyMDY4NjcwMzY3fQ.zLhFm0aPCDfbUQHWWdPJeRfXaI06JP1sHzfGdHM0n9g';
...
export async function getUserBalance(userId: string | number) {
  try {
    ...
    if (depositError || withdrawalError) {
      console.error('Error fetching transaction data:', depositError || withdrawalError);
    }

    // Calculate totals
    const totalDeposits = deposits?.reduce((sum, item) => sum + (Number(item.amount) || 0), 0) || 0;
    const totalWithdrawals = withdrawals?.reduce((sum, item) => sum + (Number(item.amount) || 0), 0) || 0;
...
export async function createDeposit(userId: string | number, amount: number, method: string, reference: string) {
  try {
    const { data, error } = await supabase
      .from('deposits')
      .insert([
...
export async function createWithdrawal(userId: string | number, amount: number, method: string, address: string) {
  try {
    const { data, error } = await supabase
      .from('withdrawals')
      .insert([
