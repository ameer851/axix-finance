...
const LanguageDropdown: React.FC<LanguageDropdownProps> = ({ 
  ...
  const includedLanguages = [
    'en', 'es', 'fr', 'de', 'it', 'pt', 'ru', 'ja', 'ko', 'zh-CN', 'zh-TW', 'ar', 'hi',
    'th', 'vi', 'tr', 'pl', 'nl', 'sv', 'no', 'da', 'fi', 'el', 'he', 'fa', 'ur', 'bn',
    'ta', 'te', 'ml', 'kn', 'gu', 'pa', 'or', 'as', 'ne', 'si', 'my', 'km', 'lo', 'ka',
    'am', 'sw', 'zu', 'af', 'sq', 'az', 'be', 'bg', 'ca', 'hr', 'cs', 'et', 'tl', 'gl',
    'hu', 'is', 'id', 'ga', 'lv', 'lt', 'mk', 'ms', 'mt', 'ro', 'sr', 'sk', 'sl', 'uk',
    'cy', 'eu', 'hy', 'kk', 'ky', 'lb', 'mn', 'uz', 'yo', 'ig', 'ha', 'mg', 'sm', 'gd',
    'mi', 'haw', 'ceb', 'ny', 'st', 'sn', 'xh', 'fy', 'hmn', 'ku', 'la', 'co', 'eo'
  ].join(',');
  ...
  useEffect(() => {
    ...
    const initializeTranslate = () => {
      if (window.google && window.google.translate && window.google.translate.TranslateElement && translateRef.current) {
        try {
  ...
  return (
    <div className={`language-dropdown-container ${variant}`}>
      {showIcon && (
        <Globe className="language-dropdown-icon" size={16} />
      )}
      <div 
        ref={translateRef}
