...
import { AlertTriangle, DollarSign, TrendingUp, TrendingDown, Users, Clock, CheckCircle, XCircle, Check, X, Trash2 } from 'lucide-react';
...
export default function WithdrawalsPage() {
  ...
  const { data: withdrawalsResponse, isLoading, isError, error } = useQuery({
  ...
  const withdrawals = React.useMemo(() => {
    ...
    if (withdrawalsResponse.withdrawals && Array.isArray(withdrawalsResponse.withdrawals)) {
      console.log('Found withdrawals property:', withdrawalsResponse.withdrawals.length);
      return withdrawalsResponse.withdrawals;
    }
    
    // Try to find any array property that might contain our withdrawals
    const arrayProps = Object.entries(withdrawalsResponse)
      .find(([key, value]) => Array.isArray(value) && 
        (key === 'data' || key === 'withdrawals' || key === 'transactions' || key === 'results'));
      
    if (arrayProps) {
      console.log(`Found array in property ${arrayProps[0]}:`, arrayProps[1].length);
      return arrayProps[1];
    }
    
    // Last resort: look for any array
    const anyArray = Object.values(withdrawalsResponse).find(v => Array.isArray(v));
    if (anyArray) {
      console.log('Found some array property:', anyArray.length);
      return anyArray;
    }
    
    console.warn('Could not find withdrawals in API response:', withdrawalsResponse);
    return [];
  }, [withdrawalsResponse]);
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-withdrawals'] });
      queryClient.invalidateQueries({ queryKey: ['admin-stats'] });
      toast({ title: 'Success', description: 'Withdrawal rejected successfully' });
    },
    onError: () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-withdrawals'] });
      queryClient.invalidateQueries({ queryKey: ['admin-stats'] });
      toast({ title: 'Success', description: 'Withdrawal deleted successfully' });
    },
    onError: () => {
  ...
  const bulkMutation = useMutation({
    mutationFn: ({ action, withdrawalIds }: { action: string; withdrawalIds: number[] }) => {
  ...
  const handleBulkAction = (action: string) => {
    if (selectedWithdrawals.length === 0) {
      toast({ title: 'Error', description: 'Please select withdrawals first', variant: 'destructive' });
      return;
    }
    bulkMutation.mutate({ action, withdrawalIds: selectedWithdrawals.map(id => parseInt(id)) });
  };
  ...
  const filteredWithdrawals = withdrawals.filter((withdrawal: any) => {
    ...
    if (filters.amountMin && withdrawal.amount < parseFloat(filters.amountMin)) return false;
    if (filters.amountMax && withdrawal.amount > parseFloat(filters.amountMax)) return false;
    return true;
  });
  ...
  if (isError) {
    return (
      <div className="flex flex-col justify-center items-center h-64 space-y-4">
        ...
        <h2 className="text-xl font-semibold text-red-500">Error Loading Withdrawals</h2>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">${stats?.withdrawals?.total?.toLocaleString() || '0'}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats?.withdrawals?.pending || 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats?.withdrawals?.approved || 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">${stats?.withdrawals?.thisMonth?.toLocaleString() || '0'}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <TransactionFilter
        onFilter={(newFilters) => {
          setFilters(prev => ({
            ...
            dateFrom: newFilters.dateRange.from ? newFilters.dateRange.from.toISOString().split('T')[0] : '',
            dateTo: newFilters.dateRange.to ? newFilters.dateRange.to.toISOString().split('T')[0] : '',
        ...
        actions={[
          {
            ...
            variant: 'success',
            action: async (ids) => {
              await bulkMutation.mutateAsync({ action: 'approve', withdrawalIds: ids.map(id => parseInt(id)) });
            }
          },
          {
            id: 'reject', 
            ...
            variant: 'warning',
            action: async (ids) => {
              await bulkMutation.mutateAsync({ action: 'reject', withdrawalIds: ids.map(id => parseInt(id)) });
            }
          },
          {
            id: 'delete',
            ...
            variant: 'danger',
            action: async (ids) => {
              await bulkMutation.mutateAsync({ action: 'delete', withdrawalIds: ids.map(id => parseInt(id)) });
            }
          }
        ]}
      />
                ...
                {filteredWithdrawals.map((withdrawal: any) => (
                  <tr key={withdrawal.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <input
                        type="checkbox"
                        checked={selectedWithdrawals.includes(withdrawal.id.toString())}
                    ...
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">
                      {withdrawal.user?.username || `User ${withdrawal.user_id}`}
                    </td>
