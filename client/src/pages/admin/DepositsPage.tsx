...
import { AlertTriangle, DollarSign, TrendingUp, Users, Clock, CheckCircle, XCircle, Check, X, Trash2 } from 'lucide-react';
...
export default function DepositsPage() {
  ...
  const { data: depositsData = { deposits: [], totalPages: 0, currentPage: 1, totalDeposits: 0 }, isLoading, error } = useQuery({
    ...
    queryFn: () => {
      ...
      return adminService.getDeposits(apiFilters);
    },
  ...
  const deposits = Array.isArray(depositsData?.deposits) ? depositsData.deposits : [];
  ...
  const bulkMutation = useMutation({
    mutationFn: ({ action, depositIds }: { action: string; depositIds: number[] }) => {
  ...
  const handleBulkAction = (action: string) => {
    if (selectedDeposits.length === 0) {
      toast({ title: 'Error', description: 'Please select deposits first', variant: 'destructive' });
      return;
    }
    bulkMutation.mutate({ action, depositIds: selectedDeposits.map(id => parseInt(id)) });
  };
  ...
  const filteredDeposits = deposits.filter((deposit: any) => {
    ...
    if (filters.dateFrom && new Date(deposit.createdAt) < new Date(filters.dateFrom)) return false;
    if (filters.dateTo && new Date(deposit.createdAt) > new Date(filters.dateTo)) return false;
    if (filters.amountMin && deposit.amount < parseFloat(filters.amountMin)) return false;
    if (filters.amountMax && deposit.amount > parseFloat(filters.amountMax)) return false;
    return true;
  });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">${stats?.deposits?.total?.toLocaleString() || '0'}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats?.deposits?.pending || 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats?.deposits?.approved || 0}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">${stats?.deposits?.thisMonth?.toLocaleString() || '0'}</div>
          </CardContent>
        </Card>
      </div>
        ...
        actions={[
          {
            ...
            variant: 'success',
            action: async (ids) => {
              await bulkMutation.mutateAsync({ action: 'approve', depositIds: ids.map(id => parseInt(id)) });
            }
          },
          {
            id: 'reject', 
            ...
            variant: 'warning',
            action: async (ids) => {
              await bulkMutation.mutateAsync({ action: 'reject', depositIds: ids.map(id => parseInt(id)) });
            }
          },
          {
            id: 'delete',
            ...
            variant: 'danger',
            action: async (ids) => {
              await bulkMutation.mutateAsync({ action: 'delete', depositIds: ids.map(id => parseInt(id)) });
            }
          }
        ]}
      />
                ...
                {filteredDeposits.map((deposit: any) => (
                  <tr key={deposit.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <input
                        type="checkbox"
                        checked={selectedDeposits.includes(deposit.id.toString())}
                    ...
                    <td className="px-6 py-4 text-sm text-gray-900">
                      {deposit.plan_name && (
                        <div className="space-y-1">
                          <div className="font-medium text-blue-600">{deposit.plan_name}</div>
                          {deposit.plan_duration && (
                            <div className="text-xs text-gray-500">Duration: {deposit.plan_duration}</div>
                          )}
                        </div>
                      )}
                      {!deposit.plan_name && <span className="text-gray-400">No plan selected</span>}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-900">
                      {(deposit.daily_profit && deposit.total_return) ? (
                        <div className="space-y-1">
                          <div className="text-green-600 font-medium">
                            Total: ${parseFloat(deposit.total_return).toLocaleString()}
                          </div>
                          <div className="text-xs text-gray-500">
                            Daily: ${parseFloat(deposit.daily_profit).toLocaleString()}
                          </div>
                        </div>
                      ) : deposit.plan_name ? (
                        <div className="text-sm text-gray-500">
                          <div>Plan: {deposit.plan_name}</div>
                          {deposit.description && deposit.description.includes('Daily Profit:') && (
                            <div className="text-xs">
                              {deposit.description.split(' - ').slice(1).join(' - ')}
                            </div>
                          )}
                        </div>
                      ) : (
                    ...
                    <td className="px-6 py-4 text-sm text-gray-900">
                      {deposit.crypto_type || deposit.method || 'Account Balance'}
                    </td>
                    <td className="px-6 py-4">
