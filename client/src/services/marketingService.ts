...
const MOCK_MARKETING_MATERIALS = [
  {
    ...
    embedCode:
      '<a href="https://axix-finance.co/ref?id=fiona500&banner=1"><img src="https://via.placeholder.com/728x90?text=Axix+Finance+Banner" alt="Axix Finance" /></a>',
  ...
  },
  {
    ...
    embedCode:
      '<a href="https://axix-finance.co/ref?id=fiona500&banner=2"><img src="https://via.placeholder.com/300x250?text=Axix+Finance" alt="Axix Finance" /></a>',
...
export async function getMarketingMaterials(): Promise<any[]> {
  try {
    ...
    if (error.status === 403) {
      throw new Error(
        "You do not have permission to view marketing materials."
      );
    ...
    throw new Error(
      error.message ||
        "Failed to fetch marketing materials. Please try again later."
    );
  }
}
...
export async function getMarketingStats(
  ...
  try {
    ...
    if (error.status === 403) {
      throw new Error(
        "You do not have permission to view marketing statistics."
      );
    ...
    throw new Error(
      error.message ||
        "Failed to fetch marketing statistics. Please try again later."
    );
  }
}
...
export async function trackMarketingClick(data: {
  ...
  try {
    ...
    if (error.status === 400) {
      throw new Error(
        error.message || "Invalid tracking data. Please check your inputs."
      );
    } else if (error.isOffline || error.isNetworkError) {
