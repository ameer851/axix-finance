 ...
 */
export async function sendWelcomeEmail(user: { email: string; username: string; first_name?: string; last_name?: string }): Promise<{ success: boolean; message: string }> {
  try {
    // Since we're using Supabase directly, we need to call the server API for email sending
    const response = await fetch(`${import.meta.env.VITE_API_URL}/api/send-welcome-email`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ message: 'Failed to send welcome email' }));
