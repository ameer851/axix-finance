...
export const adminService = {
  ...
  getDeposits: async (filters: FilterOptions & PaginationOptions = {}) => {
    ...
    try {
      ...
      const response = await apiRequest(
        "GET",
        `/api/admin/deposits-simple?${params}`
      );
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        if (contentType && contentType.includes("text/html")) {
          throw new Error(
            "Authentication required - received HTML response (likely redirected to login)"
          );
        }
  ...
  getWithdrawals: async (filters: FilterOptions & PaginationOptions = {}) => {
    ...
    console.log(
      "Fetching withdrawals with params:",
    ...
    try {
      const response = await apiRequest(
        "GET",
        `/api/admin/withdrawals-simple?${params}`
      );
